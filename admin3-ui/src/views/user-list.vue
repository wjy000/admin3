                ...
                <el-col :xl="18" :lg="18" style="border-right: 1px solid #dcdfe6">
                    <div style="padding-left: 10px">
                        <div class="handle-box">
                            <el-input v-model="query.username" placeholder="用户名" class="handle-input mr10"
                                      clearable></el-input>
                            ...
                            <el-button type="primary" :icon="Plus"
                                       @click="addVisible = true;Object.assign(form, new User())"
                            ...
                            <el-table-column label="头像" align="center">
                                <template #default="scope">
                                    <el-image
                                        ...
                                        :src="`${BASE_URI}/`+scope.row.avatar"
                                        :z-index="10"
                                        :preview-src-list="BASE_URI+'/'+[scope.row.avatar]"
                                        preview-teleported
                                    >
                                    </el-image>
                                </template>
                            ...
                            <el-table-column label="操作" width="300" fixed="right">
                                <template #default="scope">
                                    <el-popconfirm title="确定要禁用?" @confirm="handleDisable(scope.row)">
                                        <template #reference>
                                            <el-button v-if="scope.row.state==='NORMAL'" text :icon="Lock"
                                                       v-action:user:update>
                                                禁用
                                            </el-button>
                                        </template>
                                    </el-popconfirm>
                                    <el-popconfirm title="确定要启用?" @confirm="handleEnable(scope.row)">
                                        <template #reference>
                                            <el-button v-if="scope.row.state==='LOCKED'" text :icon="Unlock"
                                                       v-action:user:update>
                                                启用
                                            </el-button>
                                        </template>
        ...
        <el-dialog title="新增" v-model="addVisible" width="30%">
            <el-form label-width="70px">
                ...
                <el-form-item label="头像">
                    <AvatarUpload :img-src="`${BASE_URI}/`+form.avatar" @on-select="onAvatarSelect"/>
                </el-form-item>
            </el-form>
            <template #footer>
				<span class="dialog-footer">
